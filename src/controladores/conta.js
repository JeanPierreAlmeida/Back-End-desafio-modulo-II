...
function numeroDeContaInvalida(numeroConta) {
  if (numeroConta === '' || numeroConta === ' ' || Number.isInteger(numeroConta) || (Number(numeroConta) % 1 !== 0)) {
    return true;
  }
  ...
  if (contas.length !== 0) {
    for (const conta of contas) {
      ...
      if (cpf === requisicao.body.cpf) {
        if (requisicao.method === 'POST' || ((requisicao.method === 'PUT') && (requisicao.params.numeroConta !== conta.numero_conta))) {
...
const listarContas = async (req, res) => {
  ...
  if (req.query.senha !== senhaGerente) {
    res.status(400);
...
const atualizarUsuarioConta = async (req, res) => {
  ...
  const erro = validarCliente({
    ...
    data_nascimento: req.body.data_nascimento !== undefined
      ? req.body.data_nascimento : cliente.usuario.data_nascimento,
...
const excluirConta = async (req, res) => {
  ...
  const cliente = contas.find((conta, index) => {
    ...
    return conta.numero_conta === req.params.numeroConta;
  });
